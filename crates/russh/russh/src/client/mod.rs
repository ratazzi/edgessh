...
#[derive(Debug)]
pub enum DisconnectReason<E: From<crate::Error> + crate::MaybeSend> {
...
#[cfg(not(target_arch = "wasm32"))]
pub async fn connect<H: Handler + crate::MaybeSend + 'static, A: tokio::net::ToSocketAddrs>(
    config: Arc<Config>,
    addrs: A,
    handler: H,
...
where
    H: Handler + crate::MaybeSend + 'static,
    R: AsyncRead + AsyncWrite + Unpin + crate::MaybeSend + 'static,
{
    // Writing SSH id.
...
impl Session {
    ...
    async fn run<H: Handler + crate::MaybeSend, R: AsyncRead + AsyncWrite + Unpin + crate::MaybeSend>(
        mut self,
        stream: SshRead<R>,
        mut handler: H,
    ...
    #[cfg(not(target_arch = "wasm32"))]
    async fn run_inner<H: Handler + crate::MaybeSend, R: AsyncRead + AsyncWrite + Unpin + crate::MaybeSend>(
        &mut self,
...
pub trait Handler: Sized + crate::MaybeSend {
    type Error: From<crate::Error> + crate::MaybeSend + core::fmt::Debug;
    ...
    fn auth_banner(
        ...
        banner: &str,
        session: &mut Session,
    ) -> impl Future<Output = Result<(), Self::Error>> + crate::MaybeSend {
        async { Ok(()) }
    }
    ...
    fn should_accept_unknown_server_channel(
        ...
        id: ChannelId,
        channel_type: &str,
    ) -> impl Future<Output = bool> + crate::MaybeSend {
        async { false }
    }
